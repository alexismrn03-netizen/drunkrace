...
export default function GroupLobby({ user, profile, onJoinGroup, onProfileUpdate }: any) {
  ...
  const createGroup = async () => {
    ...
    const { data, error: e } = await supabase.from("groups").insert({
      ...
      creator_id: user.id, creator_pseudo: profile.pseudo,
      status: "waiting"
    }).select().single()
  ...
  const joinByCode = async () => {
    ...
    if (!existing) {
      const colors = ["#ec4899","#38bdf8","#4ade80","#fb923c","#818cf8","#34d399"]
      await supabase.from("group_members").insert({
        group_id: grp.id, user_id: user.id,
        color: colors[Math.floor(Math.random()*colors.length)]
      })
    }
    if (grp.status === "active") onJoinGroup(grp)
    else { setError("La course n'a pas encore démarré. Attends que le créateur lance !"); setLoading(false) }
  }
  ...
  const S = {
    ...
    btn: (color?: string) => ({ width:"100%",padding:"13px",borderRadius:13,border:"none",cursor:"pointer",background:color||"linear-gradient(135deg,#a855f7,#ec4899)",color:"#fff",fontSize:14,fontWeight:700,fontFamily:"'Space Grotesk',sans-serif" }),
    label: { fontSize:11,color:"#9ca3af",fontWeight:700 as const,letterSpacing:1,textTransform:"uppercase" as const,display:"block",marginBottom:6 },
  }

  // If group just created, show QR
  if (newGroup) {
    ...
    return (
      <div style={{ padding:"24px 16px",maxWidth:480,margin:"0 auto" }}>
        ...
        <div style={{ ...S.card,textAlign:"center" }}>
          ...
          <div style={{ fontFamily:"'Bebas Neue',cursive",fontSize:24,color:"#e2e8f0",letterSpacing:2,marginBottom:4 }}>{newGroup.name}</div>
          <div style={{ fontSize:12,color:"#6b7280",marginBottom:20 }}>Partage le QR code à tes amis !</div>
          <div style={{ background:"#fff",borderRadius:16,padding:16,display:"inline-block",marginBottom:16 }}>
            <QRCode value={joinUrl} size={180} />
          </div>
          <div style={{ background:"#1e1e2e",borderRadius:12,padding:"10px 16px",marginBottom:16 }}>
            <div style={{ fontSize:11,color:"#6b7280",marginBottom:4 }}>Code d'invitation</div>
            <div style={{ fontFamily:"'Bebas Neue',cursive",fontSize:28,color:"#c084fc",letterSpacing:4 }}>{newGroup.join_code}</div>
          </div>
          <div style={{ fontSize:11,color:"#4b5563",marginBottom:20,wordBreak:"break-all" as const }}>{joinUrl}</div>
  ...
  return (
    <div style={{ padding:"24px 16px 40px",maxWidth:480,margin:"0 auto" }}>
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24 }}>
        ...
        <button onClick={logout} style={{ background:"#1e1e2e",border:"1px solid #2a2a3e",borderRadius:10,color:"#6b7280",fontSize:12,padding:"8px 12px",cursor:"pointer",fontFamily:"'Space Grotesk',sans-serif" }}>
          Déco
        </button>
      </div>
      ...
      {creating ? (
        <div style={S.card}>
          <label style={S.label}>Nom de la soirée</label>
          <input style={S.input} value={groupName} onChange={e=>setGroupName(e.target.value)} placeholder="Ex: Soirée IUT 🎉" maxLength={40} onKeyDown={e=>e.key==="Enter"&&createGroup()} />
        ...
        <button onClick={()=>setCreating(true)} style={{ ...S.btn(),marginBottom:12 }}>
          🎉 Créer une soirée
        </button>
      )}
      ...
      {/* Join by code */}
      <div style={S.card}>
        <label style={S.label}>Rejoindre avec un code</label>
        <input style={S.input} value={joinCode} onChange={e=>setJoinCode(e.target.value.toUpperCase())} placeholder="CODE123" maxLength={6} />
        {error && <div style={{ color:"#ef4444",fontSize:12,marginBottom:8 }}>{error}</div>}
        <button onClick={joinByCode} disabled={loading} style={S.btn("#1f2937")}>
          🚀 Rejoindre la course
        </button>
      </div>
      ...
      {groups.length > 0 && (
        <div style={S.card}>
          <label style={S.label}>Soirées passées</label>
          {groups.map(g=>(
            <div key={g.id} style={{ display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:"1px solid #1e1e2e" }}>
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:10,color:g.status==="active"?"#4ade80":"#6b7280" }}>{g.status==="active"?"🟢 En cours":"⚫ Terminé"}</div>
              </div>
              {g.status==="active" && (
                <button onClick={()=>onJoinGroup(g)} style={{ background:"#166534",border:"none",borderRadius:8,color:"#4ade80",fontSize:11,padding:"5px 10px",cursor:"pointer" }}>
                  Rejoindre
                </button>
              )}
            </div>
