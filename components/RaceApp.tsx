...
import { DRINK_BASES, calcCurrentBAC, getBACStatus, cmToMeters, type DrinkEntry } from "@/lib/drinks"
...
// ── RACE TRACK ───────────────────────────────────────────────────────────────
function RaceTrack({ members, samMember, isCreator, onShowWheel, onRemoveSam, onEndRace }: any) {
  ...
  const sorted   = [...drinkers].sort((a:any,b:any) =>
    b.drinks.reduce((s:number,d:DrinkEntry)=>s+cmToMeters(d.vol_cl),0) - a.drinks.reduce((s:number,d:DrinkEntry)=>s+cmToMeters(d.vol_cl),0)
  )
    ...
    <div style={{ padding:"16px 16px 100px" }}>
            ...
            <div key={member.user_id} style={{ marginBottom:i<sorted.length-1?10:0 }}>
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4 }}>
                <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                  ...
                  <span style={{ fontSize:12,fontWeight:600,color:member.isMe?"#c084fc":"#d1d5db" }}>
                    {member.pseudo}{member.isMe?" (moi)":""}
                  </span>
      ...
      <div style={{ background:"#13131f",borderRadius:14,padding:14,border:"1px solid #1f2937",marginBottom:12 }}>
        ...
        {sorted.map((m:any,i:number)=>{
          ...
          const bac=calcCurrentBAC(m.drinks,m.weight_kg,m.sex)
          const st=getBACStatus(bac)
        ...
        {samMember && (
          <div style={{ display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderTop:"1px solid #1a1a2a",marginTop:4 }}>
            ...
            <span style={{ fontSize:20 }}>{samMember.avatar}</span>
            <div style={{ flex:1 }}>
              <div style={{ fontWeight:600,fontSize:13,color:"#4ade80" }}>{samMember.pseudo}</div>
      ...
      {isCreator && (
        <button onClick={onEndRace} style={{ width:"100%",padding:"14px",borderRadius:14,border:"1px solid #7f1d1d",cursor:"pointer",background:"#1c0505",color:"#f87171",fontSize:14,fontWeight:700 }}>
          🏁 Terminer la soirée
        </button>
      )}
    </div>
...
function StatsTab({ myMember, members, samMember }: any) {
  const { calcPeak, calcSoberTime, fmtTime } = require("@/lib/drinks")
  ...
  const drinkCounts = (myMember.drinks as DrinkEntry[]).reduce((acc: any[], d) => {
    const ex = acc.find(x => x.drinkId === d.drinkId && x.vol_cl === d.vol_cl)
    if (ex) ex.count++
    ...
    <div style={{ padding:"16px 16px 100px" }}>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14 }}>
        {[
          ...
          {l:"Calories",   v:`~${(totalAlc*7).toFixed(0)} kcal`,icon:"🔥",c:"#38bdf8"},
        ].map(s=>(
      ...
      {myMember.drinks.length > 0 && (
        <div style={{ background:"#13131f",borderRadius:14,padding:14,marginBottom:14,border:"1px solid #2a2a3e" }}>
          ...
          <div style={{ display:"flex",gap:8 }}>
            <div style={{ flex:1,background:"#1a2e0a",border:"1px solid #166534",borderRadius:10,padding:"10px" }}>
              <div style={{ fontSize:9,color:"#4ade80",fontWeight:700,textTransform:"uppercase" as const }}>🚗 Conduire</div>
              <div style={{ fontSize:16,fontWeight:700,color:"#4ade80",fontFamily:"'Bebas Neue',cursive",marginTop:2 }}>
                {driveTime && driveTime > Date.now() ? fmtTime(driveTime) : "✅ OK"}
              </div>
              <div style={{ fontSize:9,color:"#6b7280" }}>retour à 0,5‰</div>
            </div>
            <div style={{ flex:1,background:"#1a1a2e",border:"1px solid #3b1f6a",borderRadius:10,padding:"10px" }}>
              <div style={{ fontSize:9,color:"#c084fc",fontWeight:700,textTransform:"uppercase" as const }}>😇 Sobre</div>
              <div style={{ fontSize:16,fontWeight:700,color:"#c084fc",fontFamily:"'Bebas Neue',cursive",marginTop:2 }}>
                {soberTime && soberTime > Date.now() ? fmtTime(soberTime) : "✅ OK"}
              </div>
              <div style={{ fontSize:9,color:"#6b7280" }}>retour à 0‰</div>
            </div>
            <div style={{ flex:1,background:"#2a1a0a",border:"1px solid #92400e",borderRadius:10,padding:"10px" }}>
              <div style={{ fontSize:9,color:"#fb923c",fontWeight:700,textTransform:"uppercase" as const }}>📈 Pic</div>
              <div style={{ fontSize:16,fontWeight:700,color:"#fb923c",fontFamily:"'Bebas Neue',cursive",marginTop:2 }}>
                {fmtTime(peak.atMs)}
              </div>
      ...
      <div style={{ background:"#13131f",borderRadius:14,padding:14,marginBottom:14,border:"1px solid #2a2a3e" }}>
        <div style={{ display:"flex",justifyContent:"space-between",marginBottom:8 }}>
          <span style={{ fontSize:11,fontWeight:700,color:"#6b7280",letterSpacing:1 }}>ALCOOLÉMIE ACTUELLE</span>
...
// ── PROFILE TAB ──────────────────────────────────────────────────────────────
function ProfileTab({ myMember, user, onUpdate }: any) {
  ...
  return (
    <div style={{ padding:"16px 16px 100px" }}>
      ...
      <button onClick={logout} style={{ width:"100%",padding:"12px",borderRadius:13,border:"1px solid #2a2a3e",cursor:"pointer",background:"#1e1e2e",color:"#6b7280",fontSize:14,marginBottom:10 }}>
        Se déconnecter
      </button>
      <div style={{ background:"#13131f",borderRadius:12,padding:12,border:"1px solid #2a2a3e" }}>
        <p style={{ color:"#6b7280",fontSize:11,margin:0,lineHeight:1.7 }}>⚠️ Calcul BAC via formule de Widmark + courbe absorption 90min — estimation. Ne conduis jamais après avoir bu. 🚗🚫</p>
      </div>
    </div>
  )
}
...
function TabBar({ active, onChange }: { active: string, onChange: (t:string)=>void }) {
  ...
  return (
    <div style={{ display:"flex",position:"fixed",bottom:0,left:0,right:0,background:"#0a0a14",borderTop:"1px solid #1a1a2a",zIndex:100,maxWidth:480,margin:"0 auto" }}>
      {tabs.map(t=>(
        <button key={t.id} onClick={()=>onChange(t.id)} style={{ flex:1,padding:"11px 0 8px",background:"none",border:"none",color:active===t.id?"#c084fc":"#4b5563",fontSize:10,fontWeight:active===t.id?700:400,cursor:"pointer",display:"flex",flexDirection:"column" as const,alignItems:"center",gap:1 }}>
...
export default function RaceApp({ user, profile, group, onLeave, onProfileUpdate }: any) {
  ...
  const loadMembers = async () => {
    ...
    const enriched = membersData.map((m: any) => {
      ...
      return {
        ...
        drinks:      (m.drinks_log  || []).map((entry: any) => {
          // Support both old string IDs and new DrinkEntry objects
          if (typeof entry === "string") {
            const base = DRINK_BASES.find(b => b.id === entry)
            if (!base) return null
            const { alcoholGrams, cmToMeters: c } = require("@/lib/drinks")
            return { id: entry, drinkId: base.id, name: base.name, emoji: base.emoji, vol_cl: base.volumes[0], degree_pct: base.degree_pct, alcohol_g: alcoholGrams(base.volumes[0], base.degree_pct), color: base.color, addedAt: Date.now() }
          }
          return entry
        }).filter(Boolean),
      ...
      {[...members].filter(m=>!m.is_sam).sort((a,b)=>b.drinks.reduce((s:number,d:DrinkEntry)=>s+cmToMeters(d.vol_cl),0)-a.drinks.reduce((s:number,d:DrinkEntry)=>s+cmToMeters(d.vol_cl),0)).slice(0,3).map((m,i)=>(
        <div key={m.user_id} style={{ display:"flex",alignItems:"center",gap:12,padding:"10px 20px",background:"#13131f",borderRadius:14,marginBottom:8,width:"100%",maxWidth:320,border:"1px solid #2a2a3e" }}>
          ...
          <div style={{ fontWeight:700,color:"#c084fc",fontFamily:"'Bebas Neue',cursive",fontSize:18 }}>
            {m.drinks.reduce((s:number,d:DrinkEntry)=>s+cmToMeters(d.vol_cl),0).toFixed(1)}m
          </div>
        </div>
      ))}
  ...
  return (
    <div style={{ background:"#0a0a14",minHeight:"100vh",maxWidth:480,margin:"0 auto",position:"relative" }}>
      {tab==="race"    && <RaceTrack members={members} samMember={samMember} isCreator={isCreator} onShowWheel={()=>setShowWheel(true)} onRemoveSam={handleRemoveSam} onEndRace={handleEndRace}/>}
      ...
      {tab==="profile" && <ProfileTab myMember={myMember} user={user} onUpdate={(p:any)=>{ setMembers(prev=>prev.map(m=>m.isMe?{...m,...p}:m)); onProfileUpdate() }}/>}
