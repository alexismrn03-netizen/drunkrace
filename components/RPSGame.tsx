...
export default function RPSGame({ members, myUserId, groupId, invite, onAwardDistance, onClose }: Props) {
  ...
  // ── RESULT ────────────────────────────────────────────────────────────────
  if (phase === "result") {
    const myEm = CHOICES.find(c=>c.id===myChoice)?.emoji || "❓"
    const oppEm = CHOICES.find(c=>c.id===opponentChoice)?.emoji || "❓"
    const oppName = opponent?.pseudo || "Adversaire"
    return (
      <div style={BG}>
        <div style={{ textAlign:"center", marginBottom:24, width:"100%", maxWidth:340 }}>
          <div style={{ fontFamily:"'Bebas Neue',cursive", fontSize:32, letterSpacing:3, color:result==="win"?"#4ade80":result==="draw"?"#fbbf24":"#f87171", marginBottom:8 }}>
          ...
          {/* Delta */}
          <div style={{ background:result==="win"?"#052e16":result==="draw"?"#1a1030":"#1c0505", border:`1px solid ${result==="win"?"#166534":result==="draw"?"#3b1f6a":"#7f1d1d"}`, borderRadius:14, padding:"12px 20px", marginBottom:24 }}>
            <div style={{ fontSize:14, fontWeight:700, color:result==="win"?"#4ade80":result==="draw"?"#c084fc":"#f87171" }}>
              {result==="win"?"+8m sur la piste 🏁":result==="draw"?"+2m chacun 🤝":"−3m + bois un shot 🥃"}
            </div>
          </div>
        </div>
