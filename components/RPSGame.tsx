...
export default function RPSGame({ members, myUserId, groupId, invite, onAwardDistance, onClose }: Props) {
  ...
  // Poll for invite updates every 1.5s
  useEffect(() => {
    if (!inviteId) return
    const poll = async () => {
      const { data } = await supabase.from("game_invites").select("*").eq("id", inviteId).single()
      if (!data) return
      // Challenger: opponent accepted
      if (isChallenger && data.status === "accepted") {
        setPhase(prev => prev === "waiting_accept" ? "ready_p1" : prev)
      }
      // Challenger: opponent chose → compute
      if (isChallenger && data.game_data?.p2_choice) {
        const p2c = data.game_data.p2_choice
        setPhase(prev => {
          if (prev === "waiting_p2") {
            setOpponentChoice(p2c)
            computeResult(p1ChoiceRef.current, p2c)
          }
          return prev
        })
      }
      // Both completed
      if (data.status === "completed" && data.game_data?.p1_choice && data.game_data?.p2_choice) {
        const p1c = data.game_data.p1_choice
        const p2c = data.game_data.p2_choice
        const w = getWinner(p1c, p2c)
        const iWin = isChallenger ? w === "p1" : w === "p2"
        if (isChallenger) setMyChoice(p1c)
        setOpponentChoice(isChallenger ? p2c : p1c)
